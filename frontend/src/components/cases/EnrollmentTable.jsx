...
export default function EnrollmentTable({ data = [], loading = false }) {
  ...
  const sortedData = useMemo(() => {
    ...
    return [...filteredData].sort((a, b) => {
      ...
      // S29 FIX: Numeric sort for Age, SumInsured, Claims, and amount columns
      ...
      if (!isNaN(aNum) && !isNaN(bNum) && String(aVal).trim() !== '' && String(bVal).trim() !== '') {
        ...
        // String comparison for non-numeric fields (Name, Gender, etc.)
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
      <div className="p-4 border-b border-gray-200 bg-gray-50">
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1 relative">
            ...
            <input type="text" placeholder="Search all columns..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
      ...
      <div className="overflow-x-auto">
        {loading ? <div className="p-8 text-center text-gray-500">Loading enrollment data...</div> : paginatedData.length === 0 ? (
          <div className="p-8 text-center text-gray-500">{data.length === 0 ? 'No enrollment data' : 'No matching records'}</div>
        ) : (
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {paginatedData.map((row, i) => (
                <tr key={i} className="hover:bg-gray-50">
                  {COLUMNS.map(col => {
                    const v = row[col.key], dv = col.format ? col.format(v) : v || '-';
                    return (
                      <td key={col.key} className="px-4 py-3 text-sm text-gray-700">
                        {col.key === 'Chronic_Condition' ? (
                          <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${v ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>{dv}</span>
                        ) : dv}
                      </td>
                    );
                  })}
